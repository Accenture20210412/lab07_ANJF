...
public class PITCalculator {
    ...
    private boolean taxIsNotApplicableFor(BigDecimal taxBase) {
        return BigDecimalComparison.is(taxBase).notGreaterThan(firstTaxThreshold);
    }

    private boolean firstTaxRateIsApplicableFor(BigDecimal taxBase) {
        return BigDecimalComparison.is(taxBase).between(firstTaxThreshold).andInclusive(secondTaxThreshold);
    }
    ...
    private boolean secondTaxRateIsApplicableFor(BigDecimal taxBase) {
        return BigDecimalComparison.is(taxBase).between(secondTaxThreshold).andInclusive(solidarityTaxThreshold);
    }
    ...
    private BigDecimal deductionForFirstTaxThreshold(BigDecimal taxBase) {
        return BigDecimalComparison.is(taxBase).notGreaterThan(valueOf(13000))
    ...
    private BigDecimal deductionForSecondTaxThreshold(BigDecimal taxBase) {
        return BigDecimalComparison.is(taxBase).notGreaterThan(valueOf(127000))
